...
exports.mockTrips = [
    {
        ...
        description: "Explore one of the most spectacular rock formations in Africa. Trek through ancient plateaus, discover prehistoric cave paintings, and experience authentic Tuareg culture under the stars.",
        category: "desert",
        difficulty: "Moderate",
    ...
    },
    {
        ...
        description: "Discover the world's largest rainforest. Navigate the mighty Amazon River, spot exotic wildlife, and learn about sustainable tourism from expert naturalists.",
        category: "jungle",
        difficulty: "Hard",
        cover_image: "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=800&h=600&fit=crop",
    ...
    },
    {
        ...
        description: "Climb Africa's highest peak. A challenging but rewarding journey to 5,895m with acclimatization stages and experienced mountaineering support.",
        category: "mountain",
        difficulty: "Expert",
        cover_image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&h=600&fit=crop",
    ...
    },
    {
        ...
        description: "Experience the majestic fjords of Norway. Hike through stunning mountain landscapes, stay in cozy mountain lodges, and witness the midnight sun.",
        category: "mountain",
        difficulty: "Easy",
        cover_image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&h=600&fit=crop",
    ...
    },
    {
        ...
        description: "Immerse yourself in Moroccan culture. Explore ancient medinas, ride camels in the Sahara Desert, and stay with local families.",
        category: "desert",
        difficulty: "Easy",
        cover_image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&h=600&fit=crop",
...
function getAllTrips(limit = 12, offset = 0) {
    const total = exports.mockTrips.length;
    const data = exports.mockTrips.slice(offset, offset + limit);
...
function getTripById(id) {
    const trip = exports.mockTrips.find((t) => t.id === id);
    return trip || null;
}
function searchTrips(query, limit = 12, offset = 0) {
    const filtered = exports.mockTrips.filter((trip) => trip.title.toLowerCase().includes(query.toLowerCase()) ||
...
function filterTrips(filters, limit = 12, offset = 0) {
    let filtered = exports.mockTrips;
    if (filters.category?.value) {
        filtered = filtered.filter((t) => t.category?.toLowerCase() === filters.category.value.toLowerCase());
    }
    if (filters.difficulty?.value) {
        filtered = filtered.filter((t) => t.difficulty?.toLowerCase() === filters.difficulty.value.toLowerCase());
    }
    if (filters.price?.value) {
        const [minPrice, maxPrice] = filters.price.value.split(":").map(Number);
        filtered = filtered.filter((t) => (t.price || 0) >= minPrice && (t.price || 0) <= maxPrice);
    }
    ...
    if (filters.available_spots?.value) {
        ...
        filtered = filtered.filter((t) => ((t.max_participants || 0) - (t.min_participants || 0)) >= minSpots);
    }
