...
async function seed() {
    ...
    if (cpErr)
        console.warn('  Client profile upsert warnings:', cpErr.message);
    ...
    const services = [
        {
            ...
            location: 'SRID=4326;POINT(5.5 22.8)',
        ...
        },
        {
            ...
            location: 'SRID=4326;POINT(2.8 36.5)',
        },
        {
            owner_id: ownerId,
            ...
            location: 'SRID=4326;POINT(3.0 36.7)',
        },
        {
            owner_id: ownerId,
            ...
            location: 'SRID=4326;POINT(3.7 32.5)',
        },
        {
            owner_id: ownerId,
            ...
            location: 'SRID=4326;POINT(9.5 24.5)',
            availability: ['weekend'],
            details_informations: [],
            procedure: {},
            procedure_info: [],
    ...
    if (delSvcErr)
        console.warn('  Could not clear existing services:', delSvcErr.message);
    const { data: insertedServices, error: svcErr } = await supabase
        .from('services')
        .insert(services)
        .select();
    if (svcErr)
        throw new Error(`Failed to insert services: ${svcErr.message}`);
    ...
    if (revErr)
        console.warn('  Review insert warnings:', revErr.message);
    else
    ...
    if (bkErr)
        console.warn('  Booking insert warnings:', bkErr.message);
    else
    ...
    if (delTripErr)
        console.warn('  Could not clear existing trips:', delTripErr.message);
    const trips = [
        {
            organizer: ownerId,
    ...
    const { data: insertedTrips, error: tripErr } = await supabase
        ...
        .select();
    if (tripErr)
        throw new Error(`Failed to insert trips: ${tripErr.message}`);
    ...
    if (stopsErr)
        console.warn('  Trip stops warnings:', stopsErr.message);
    else
        console.log(`Inserted ${tripStops.length} trip stops`);
    const participants = [];
    ...
    if (partErr)
        console.warn('  Participant insert warnings:', partErr.message);
    else
        console.log(`Inserted ${participants.length} trip participants`);
    console.log('\n✓ Seeding complete!');
...
//# sourceMappingURL=seed-business.js.map