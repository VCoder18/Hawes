...
export function TripDetailsMap({ trip }: TripDetailsMapProps) {
  ...
  const points = useMemo<MapPoint[]>(() => {
    ...
    return stops
      .map((stop, index) => {
        const parsed = parseDestinationCoordinates(stop as any);
        const lat = Number(parsed?.lat ?? (stop as any).location?.lat);
        const lng = Number(parsed?.lng ?? (stop as any).location?.lng);
        const stopType = String((stop as any).type || (stop as any).stop_type || "meeting").toLowerCase();
